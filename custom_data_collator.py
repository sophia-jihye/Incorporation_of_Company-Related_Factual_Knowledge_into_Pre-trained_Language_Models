...
# https://github.com/huggingface/transformers/blob/v4.16.2-release/src/transformers/data/data_collator.py

# Added code
#     find_sub_list_indices
#     from dataclasses import field
#     DataCollatorForWholeWordMask.__init__
# Modified code
#     DataCollatorForWholeWordMask._whole_word_mask
...
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling):
    ...
    def __init__(self, tokenizer, mlm, mlm_probability, company_names):
        ...
        print('mlm_probability:', mlm_probability)
        pad_to_multiple_of: Optional[int] = None
        tf_experimental_compile: bool = False
        return_tensors: str = "pt"
    ...
    def _whole_word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        # 기업명 토큰 그룹
        # Ex. "Nvidia, Inc. She works at Nvidia, Inc. and Tesla, Inc."
        range_list_of_indices = [] # [(1, 6), (10, 15), (17, 20)]
        
        # speed up을 위해 꼭 필요한 소수의 company_names만 식별
        text = self.tokenizer.convert_tokens_to_string(input_tokens)
        company_names_selected = list(filter(lambda x: x.lower() in text.lower(), self.company_names))

        for name in company_names_selected: 
            range_list_of_indices.extend(find_sub_list_indices(self.company_name_to_tokens[name], input_tokens))
        
        grouped_indices = [] # [[1, 2, 3, 4, 5, 6], [10, 11, 12, 13, 14, 15], [17, 18, 19, 20]]
        for (start_idx, end_idx) in range_list_of_indices:
            grouped_indices.append(list(range(start_idx, end_idx+1)))
        random.shuffle(grouped_indices) # [[17, 18, 19, 20], [1, 2, 3, 4, 5, 6], [10, 11, 12, 13, 14, 15]]
        ...
        for index_set in cand_indexes:
            ...
            # Break option
            # num_to_predict만큼의 토큰들을 masking 완료한 경우
            if len(masked_lms) >= num_to_predict:
                break
            
            # Skip option 
            # "한 그룹에 속한 토큰 수 > num_to_predict"일 경우, 이번 그룹은 skip하고 다음 그룹으로 넘어감
            ...
            # Skip option
            # 이전 그룹에서 이미 masking된 토큰이 있다면, 이번 그룹은 skip하고 다음 그룹으로 넘어감
            ...
            # Masking phase
            # cand_indexes가 이미 셔플링된 array이기 때문에 앞에서부터 순서대로 mask해도 됨.
            for index in index_set:
